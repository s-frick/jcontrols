...
public sealed interface Either<E, A> {
   ...
   * @param success the success value from which the Either will be consutructed.
   * @return
   */
   ...
   */
  Either<E, A> orElse(Either<E, A> other);

  /**
   * @param other a {@link Supplier} that returns an Either
   * @return other Either, if this Either contains a failure value. Otherwise if this Either.
   */
  Either<E, A> orElse(Supplier<? extends Either<E, A>> other);
  ...
  record Failure<E, A>(E value) implements Either<E, A> {
    ...
    @Override
    public Either<E, A> orElse(Either<E, A> other) {
      return other;
    }

    @Override
    public Either<E, A> orElse(Supplier<? extends Either<E, A>> other) {
      return other.get();
    }
